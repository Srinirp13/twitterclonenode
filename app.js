...
const getFollowingPeopleIds0fUser = async (username) => {
...
app.get("/user/tweets/feed/", authentication, async (request, response) => {
  ...
  const followingPeopleIds = await getFollowingPeopleIds0fUser(username);
...
app.get("/user/following/", authentication, async (request, response) => {
  ...
  const followingPeople = await db.all(userFollowsQuery);
...
app.get("/user/tweets/", authentication, async (request, response) => {
  ...
  const getTweetsDetailsQuery = `
        ...
        GROUP BY
            tweet.tweet_id;`;
  const tweets = await db.all(getTweetsDetailsQuery);
  response.send(tweets);
});
